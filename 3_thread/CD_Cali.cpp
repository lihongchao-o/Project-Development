#include "CD_TxTable.h"
#include "CT_TxTable.h"
#include <QMessageBox>

const QString CD_TxTable::m_Title = QString("Cal_1");

CD_TxTable::CD_TxTable(QWidget *parent)
	: QDialog(parent)
	, ui(new Ui::CD_TxTable)
	, m_worker(NULL)
{
	ui->setupUi(this);
...
CD_TxTable::~CD_TxTable()
{
	delete ui;
}

void CD_TxTable::on_startPushButton_clicked()
{
	if (m_worker) {
		...
		m_worker = new CalThread_TxTable(this);
...
void CD_TxTable::success() {
	m_calFile->save();
}

void CD_TxTable::on_thread_done(int result) {
	success();
	ui->startPushButton->setText("开始");
...
void CD_TxTable::on_thread_error(QString message) {
	QMessageBox::critical(this, m_Title, message);
	ui->startPushButton->setText("开始");
}

void CD_TxTable::on_thread_addPlot(QString s, double x, double y) {
...
void CD_TxTable::on_plot_plottableDoubleClick(QCPAbstractPlottable *plot, QMouseEvent *e) {
	(void)e;
